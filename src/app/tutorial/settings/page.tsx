    ...
    return (
        <main className="flex min-h-screen flex-col items-center justify-center p-4">
            <Card className="w-full max-w-md shadow-2xl" data-tutorial-id="settings-card">
                <CardHeader>
                    <CardTitle className="text-center text-3xl">Ustawienia</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div data-tutorial-id="settings-switches" className="space-y-4">
                        <div className="flex items-center justify-between">
                            <Label htmlFor="sounds-switch" className="text-lg">Dźwięki</Label>
                            <Switch id="sounds-switch" checked={true} disabled />
                        </div>
                        <Separator/>
                        <div className="space-y-2">
                            <Label htmlFor="volume-slider" className="text-lg">Głośność</Label>
                            <Slider id="volume-slider" defaultValue={[50]} disabled />
                        </div>
                        <Separator/>
                        <div className="flex items-center justify-between">
                            <Label htmlFor="vibrations-switch" className="text-lg">Wibracje</Label>
                            <Switch id="vibrations-switch" checked={true} disabled />
                        </div>
                    </div>
                    <Separator />
                    <div className="space-y-2" data-tutorial-id="settings-eyecare">
                        <Label htmlFor="eyecare-slider" className="text-lg">Ochrona Wzroku</Label>
                        <Slider id="eyecare-slider" defaultValue={[20]} disabled />
                    </div>
                    <Separator />
                    <div>
                        <h3 className="text-lg font-semibold mb-2">O aplikacji</h3>
                        <div className="flex items-center justify-between">
                            <p className="text-sm text-muted-foreground pr-4">
                                Ta aplikacja buduje Twoje umiejętności językowe za pomocą szybkich i zabawnych quizów.
                            </p>
                            <Button variant="outline" size="sm" disabled>Więcej</Button>
                        </div>
                    </div>
                </CardContent>
                <CardFooter className="flex justify-center p-6 pt-2">
                    <div className="flex flex-wrap justify-center gap-4">
                        ...
                        <Button variant="destructive" disabled>
                            <Trash2 className="mr-2 h-4 w-4" /> Resetuj ustawienia
                        </Button>
                    </div>
                </CardFooter>
            </Card>
